...
return {
	...
	config = function()
		...
		-- Add a BufEnter autocommand to check if jdtls needs to be restarted
		-- when returning to a Java buffer
		vim.api.nvim_create_autocmd("BufEnter", {
			pattern = "*.java",
			group = jdtls_augroup,
			callback = function(args)
				local bufnr = args.buf
				-- Check if we're in a Java file without an active jdtls client
				local clients = vim.lsp.get_active_clients({ bufnr = bufnr, name = "jdtls" })
				if #clients == 0 then
					-- No active jdtls client, so start it
					setup_jdtls()
				end
			end,
		})
